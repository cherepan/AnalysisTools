...
TemplatesMC::TemplatesMC(TString Name_, TString id_):
  Selection(Name_,id_)
{
}

TemplatesMC::~TemplatesMC(){
  for(int j=0; j<Npassed.size(); j++){
    std::cout << "TemplatesMC::~TemplatesMC Selection Summary before: " 
	 << Npassed.at(j).GetBinContent(1)     << " +/- " << Npassed.at(j).GetBinError(1)     << " after: "
	 << Npassed.at(j).GetBinContent(NCuts) << " +/- " << Npassed.at(j).GetBinError(NCuts) << std::endl;
  }
  std::cout << "TemplatesMC::~TemplatesMC()" << std::endl;
}

void  TemplatesMC::Configure(){
  // Setup Cut Values
...
void  TemplatesMC::Store_ExtraDist(){
...
void  TemplatesMC::doEvent(){
  ...
  double wobs=1;
  double w;
  double WeightPlus =  Ntp->TauSpinerGet(TauSpinerInterface::hplus);
  double WeightMinus =  1-WeightPlus;
...
// 	ZEta.at(t).Fill(EventFitTauMu.Eta(),1);
	...
	RecoCosTheta_CosBeta_ZFrame_Plus.at(t).Fill(Helper.costheta1(OSPion_ZFrame,SSPion1_ZFrame,SSPion2_ZFrame,EventFitZ),Helper.cosbeta(SSPion1_ZFrame,SSPion2_ZFrame,OSPion_ZFrame),WeightPlus*w);
	RecoCosTheta_CosBeta_ZFrame_Minus.at(t).Fill(Helper.costheta1(OSPion_ZFrame,SSPion1_ZFrame,SSPion2_ZFrame,EventFitZ),Helper.cosbeta(SSPion1_ZFrame,SSPion2_ZFrame,OSPion_ZFrame),WeightMinus*w);
...
// 	std::cout<<"f "<<fa1 << "  ga1: " << ga1<<std::endl;
	...
	float x = 2*tempx-1;
	RecOmegaA1p.at(t).Fill(ga1/fa1,WeightPlus*w);
	RecOmegaA1m.at(t).Fill(ga1/fa1,WeightMinus*w);
	RecOmegaA1z.at(t).Fill(ga1/fa1,w);

	RecOmegaMup.at(t).Fill(x,WeightPlus*w);
	RecOmegaMum.at(t).Fill(x,WeightMinus*w);
	RecOmegaMuz.at(t).Fill(x,w);

	RecOmegaCombinedp.at(t).Fill(x+om/(1+x*om),WeightPlus*w);
	RecOmegaCombinedm.at(t).Fill(x+om/(1+x*om),WeightMinus*w);
...
double TemplatesMC::TrigEff(Double_t m, double m0, double sigma, double alpha, double n, double norm){
